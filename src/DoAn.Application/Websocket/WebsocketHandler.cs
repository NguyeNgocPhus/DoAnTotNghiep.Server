...
namespace DoAn.Application.Websocket
{
    public class WebsocketHandler
    {
        ...
        public WebsocketHandler(IRepositoryBase<Notification, int> repository)
        {
        ...
        public async Task OnConnectedAsync(WebSocket webSocket)
        {
            ...
            while (webSocket.State == WebSocketState.Open)
            {
                ...
                if (result.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    if (client_msg.command == "getCountNotification")
                    {
                        var userId = client_msg.identifier;
                        var count= await _repository.AsQueryable().Where(x => x.UserId == Guid.Parse(userId) && !x.Read).CountAsync();

                        await SendMessageToClient(userId, new { type = "getCountNotification", data = count });
                    }
                }
